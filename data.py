...
def download_shapenetpart():
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data')):
        ...
        zipfile = os.path.basename(www)
        os.system('wget --no-check-certificate %s; unzip %s' % (www, zipfile))
        os.system('mv %s %s' % ('hdf5_data', os.path.join(
            DATA_DIR, 'shapenet_part_seg_hdf5_data')))
        os.system('rm %s' % (zipfile))
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'Stanford3dDataset_v1.2_Aligned_Version')):
        if not os.path.exists(os.path.join(DATA_DIR, 'Stanford3dDataset_v1.2_Aligned_Version.zip')):
            ...
            zippath = os.path.join(
                DATA_DIR, 'Stanford3dDataset_v1.2_Aligned_Version.zip')
            os.system('unzip %s' % (zippath))
            os.system('mv %s %s' %
                      ('Stanford3dDataset_v1.2_Aligned_Version', DATA_DIR))
    ...
    if partition == 'trainval':
        file = glob.glob(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data', '*train*.h5')) \
            + glob.glob(os.path.join(DATA_DIR,
                        'shapenet_part_seg_hdf5_data', '*val*.h5'))
    else:
        file = glob.glob(os.path.join(
            DATA_DIR, 'shapenet_part_seg_hdf5_data', '*%s*.h5' % partition))
    for h5_name in file:
        f = h5py.File(h5_name, 'r+')
        data = f['data'][:].astype('float32')
        label = f['label'][:].astype('int64')
...
def load_color_partseg():
    ...
    for row in range(0, img_size):
        ...
        for column in range(0, img_size):
            ...
            if color_index >= 50:
                cv2.imwrite("prepare_data/meta/partseg_colors.png",
...
def load_color_semseg():
    ...
    for _ in range(0, img_size):
        ...
        for _ in range(0, img_size):
            ...
            label_index = label_index + 1
            if color_index >= 13:
                cv2.imwrite("prepare_data/meta/semseg_colors.png",
                            img, [cv2.IMWRITE_PNG_COMPRESSION, 0])
                return np.array(colors)
...
class ScanNet(Dataset):
    def __init__(self, num_point=8192, partition='train',
        ...
        print("Totally {} samples in {} set.".format(
            len(self.room_idxs), partition))
