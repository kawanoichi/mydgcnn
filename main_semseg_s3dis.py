...
def _init_():
    ...
    os.system('cp main_semseg_s3dis.py outputs'+'/' +
              args.exp_name+'/'+'main_semseg_s3dis.py.backup')
...
def visualization(visu, visu_format, test_choice, data, seg, pred, visual_file_index, semseg_colors):
    ...
    for i in range(0, data.shape[0]):
        ...
        if skip:
            ...
            f = open('outputs/'+args.exp_name+'/'+'visualization'+'/' +
                     'area_'+test_area+'/'+roomname+'/'+roomname+'.txt', "a")
            f_gt = open('outputs/'+args.exp_name+'/'+'visualization'+'/' +
            ...
            if roomname != roomname_next:
                mIoU = np.nanmean(calculate_sem_IoU(
                    np.array(room_pred), np.array(room_seg)))
                mIoU = str(round(mIoU, 4))
                room_pred = []
                room_seg = []
                if visu_format == 'ply':
                    filepath = 'outputs/'+args.exp_name+'/'+'visualization'+'/' + \
                    ...
                    xyzRGB = np.loadtxt('outputs/'+args.exp_name+'/'+'visualization' +
                                        '/'+'area_'+test_area+'/'+roomname+'/'+roomname+'.txt')
                    xyzRGB_gt = np.loadtxt('outputs/'+args.exp_name+'/'+'visualization' +
                    ...
                    os.system('rm -rf '+'outputs/'+args.exp_name +
                              '/visualization/area_'+test_area+'/'+roomname+'/*.txt')
                else:
                    filename = 'outputs/'+args.exp_name+'/'+'visualization' + \
                        '/'+'area_'+test_area+'/'+roomname+'/'+roomname+'.txt'
                    filename_gt = 'outputs/'+args.exp_name+'/'+'visualization' + \
                        '/'+'area_'+test_area+'/'+roomname+'/'+roomname+'_gt.txt'
                    filename_mIoU = 'outputs/'+args.exp_name+'/'+'visualization'+'/' + \
